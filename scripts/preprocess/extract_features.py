...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str,
                        default='CVPR23_DeitS_Rerank.pth', help='path to the checkpoint')
    parser.add_argument('--batch_size', type=int, default=16,
                        help='batch size for inference')
    parser.add_argument('--num_workers', type=int, default=4,
                        help='number of workers for data loading')
    parser.add_argument('--img_resize', type=int, nargs=2,
                        default=[480, 640], help='image resize')
    args = parser.parse_args()

    model = DistilledVisionTransformer(
        img_size=args.img_resize, patch_size=16, embed_dim=384,
        depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), num_classes=256)
    checkpoint = torch.load(args.checkpoint, map_location='cpu')
    state_dict = {k.replace('module.backbone.', ''): v for k,
    ...
    pbar = tqdm.trange(
        len(dataset), desc="Extracting global features", leave=False
    )
    
    with torch.inference_mode():
        for i, x in enumerate(dataloader):
            x = model(x.to(device))
            all_features[i*args.batch_size:(i+1)
                         * args.batch_size] = x.cpu().numpy()
            pbar.update(args.batch_size)

    np.save(os.path.join(args.scene_path, "features.npy"), all_features)
